...
export const joinTournament = onCall(async (request) => {
  try {
    ...
    return await db.runTransaction(async (t) => {
      ...
      // Validations
      if (tData.status !== "upcoming") throw new HttpsError("failed-precondition", "Tournament is not open for registration.");
      if (tData.currentPlayers >= tData.slots) throw new HttpsError("failed-precondition", "Tournament is full.");
      ...
      if (!currentParticipants.empty) {
        throw new HttpsError("already-exists", "User is already registered for this tournament.");
      }
      ...
      if (entryFee > 0) {
        await WalletService.deductFunds(uid, entryFee, tournamentId, { type: "TOURNAMENT_ENTRY" }, t);
      }
